...
from evaluation.evalution import eval_edge_prediction,eval_edge_prediction_auc
from model.dygprompt_downstream import DyGPrompt_downstream
...
Path("./saved_finetuning_models/").mkdir(parents=True, exist_ok=True)# 创建保存模型的文件夹
Path("./saved_finetuning_checkpoints/").mkdir(parents=True, exist_ok=True)# 创建临时保存模型的文件夹
MODEL_SAVE_PATH = f'./saved_finetuning_models/{args.prefix}-{args.data}.pth' # 最终模型文件名
get_checkpoint_path = lambda \
    epoch: f'./saved_finetuning_checkpoints/{args.prefix}-{args.data}-{epoch}.pth'# checkpoint生成名字函数
...
### 获得训练集验证集测试机数据
node_features, edge_features, full_data, train_data, val_data, test_data = get_data(DATA,mode="finetuning")
...
for i in range(args.n_runs):
  ...
  # Initialize Model
  pretrain_model=DyGPrompt_pretrain(neighbor_finder=train_ngh_finder, node_features=node_features,
            edge_features=edge_features, device=device,
            n_layers=NUM_LAYER,
            n_neighbors=NUM_NEIGHBORS,)
  pretrain_model.load_state_dict(torch.load('saved_checkpoints/tgn-attn-reddit-0.pth'))
  dygprompt = DyGPrompt_downstream(pretrain_model=pretrain_model,device=device
            )
  ...
  for epoch in range(NUM_EPOCH):
    ...
    #设置训练图的邻居查找器
    dygprompt.pretrain_model.set_neighbor_finder(train_ngh_finder)
    ...
    dygprompt.pretrain_model.set_neighbor_finder(full_ngh_finder)
    val_auc = eval_edge_prediction_auc(model=dygprompt,
    ...
    logger.info('Epoch mean loss: {}'.format(np.mean(m_loss)))
      ...
      ### Test
  dygprompt.pretrain_model.set_neighbor_finder(full_ngh_finder)
  test_auc = eval_edge_prediction_auc(model=dygprompt,
